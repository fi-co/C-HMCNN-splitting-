...
# An early attempt to parallelize the training process using google colab's GPU
# The current version of the code is not functional and is not used in the final implementation (main_split.py)
...
def main():
    ...
    parser.add_argument('--device', type=str, default=None,
                        help='GPU device ID. If None, dynamically assigns the first available GPU or falls back to CPU.')
    ####################### ---- N E W 
    ...
    if torch.cuda.is_available():
        ...
        if args.device is None or int(args.device) >= num_devices:
            print(f"No valid device ID provided or exceeding available devices. Assigning to GPU:0")
    ...
    if split_ratios:
        ...
        lengths = [int(len(train) * ratio) for ratio in split_ratios] 
        lengths.append(len(train) - sum(lengths))  # Remaining samples
        train_splits = torch.utils.data.random_split(train, lengths)
        print(f"Generated {len(train_splits)} splits: {[len(split) for split in train_splits]}")
    else:
        train_splits = [train]  # No splitting, use full dataset
