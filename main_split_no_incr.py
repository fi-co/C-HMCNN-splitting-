...
def main():
    ...
    parser.add_argument('--device', type=str, default='0',
                        help='GPU (default:0)')
    ####################### ---- N E W 
    # Additional argument for dataset splits
    parser.add_argument('--split_ratios',nargs='+', type=float, default=None,
                        help="List of ratios for incremental dataset splits, no sep")
    ####################### ---- N E W 
    ...
    split_ratios = args.split_ratios if hasattr(args, 'split_ratios') else None

    # Load the datasets
    if 'others' in args.dataset:
        train, test = initialize_other_dataset(dataset_name, datasets)
        train.to_eval, test.to_eval = (
            torch.tensor(train.to_eval, dtype=torch.uint8),
            torch.tensor(test.to_eval, dtype=torch.uint8),
        )
    else:
        train, val, test = initialize_dataset(dataset_name, datasets)
        train.to_eval, val.to_eval, test.to_eval = (
            torch.tensor(train.to_eval, dtype=torch.uint8),
            torch.tensor(val.to_eval, dtype=torch.uint8),
            torch.tensor(test.to_eval, dtype=torch.uint8),
        )
    ...
    # np.zeros(train.A.shape) initialize an empty (n x n) = R where each class is marked as its own ancestor
    # With NetworkX, R is converted into a digraph for hierarchy representation
    ...
    # Create loaders (note: loaders will be created within the split loop)
    ...
    if split_ratios:
        assert sum(split_ratios) <= 1.0, "Split ratios must sum to 1.0 or less."
        
        lengths = [int(len(train_dataset) * ratio) for ratio in split_ratios] 
        remaining = len(train_dataset) - sum(lengths)

        # Handle potential rounding issues
        if remaining > 0:
            lengths.append(remaining)
        else:
            print("No remaining samples after splitting; adjusting lengths to avoid zero-length splits.")
            # Optionally, adjust the last split to include all remaining samples
            # For example, you might remove the last split or adjust ratios
            # Here, we'll remove the last split if no remaining samples
            lengths = lengths[:-1]
    
        # Ensure no split has zero length
        if any(l == 0 for l in lengths):
            raise ValueError("One of the splits has zero length. Adjust the split ratios.")


        # perform the split
        train_splits = torch.utils.data.random_split(train_dataset, lengths)
        print(f"Generated {len(train_splits)} splits: {[len(split) for split in train_splits]}")
    else:
    ...
    ################## ----  MODIFIED TO TRAIN ON EACH SPLIT

    num_epochs_per_split = num_epochs # Keep original epochs
    ...
    for split_idx, train_subset in enumerate(train_splits): 
        ...
        for epoch in range(num_epochs_per_split):
            model.train() 
            epoch_loss = 0.0
            ...
            print(f"Epoch {epoch + 1}/{num_epochs_per_split} - Loss: {avg_epoch_loss:.4f}") 
