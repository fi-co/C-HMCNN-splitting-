...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train neutral network on train and validation set')
    ...
    parser.add_argument('--device', type=str, default='0',
                        help='GPU (default:0)')
    ####################### ---- N E W 
    # If not specified, use full dataset
    parser.add_argument('--num_splits', type=int, default=1,
                       help="Number of incremental dataset splits") 
    ####################### ---- N E W 
    ...
    # Calculate split ratios based on the number of splits
    #split_ratios = args.split_ratios
    num_splits = args.num_splits

    if num_splits: 
        assert num_splits > 0, "num_splits must be a positive number."
        split_ratios = [1.0 / num_splits] * num_splits 
        print(f"Automatically generated split_ratios based on num_splits={num_splits}: {split_ratios}")
    else:
        num_splits = None  # Use full dataset

    ########################## ----  N E W
    # Load the datasets
    if 'others' in args.dataset:
        train, test = initialize_other_dataset(dataset_name, datasets)
        train.to_eval, test.to_eval = (
            torch.tensor(train.to_eval, dtype=torch.uint8), torch.tensor(test.to_eval, dtype=torch.uint8),
        )
    else:
        train, val, test = initialize_dataset(dataset_name, datasets)
        train.to_eval, val.to_eval, test.to_eval = (
            torch.tensor(train.to_eval, dtype=torch.uint8), torch.tensor(val.to_eval, dtype=torch.uint8), torch.tensor(test.to_eval, dtype=torch.uint8),
        )

    # Checks which classes have at least one instance, i.e., their sum is ≠ 0
    ...
    if 'others' in args.dataset:
        ...
        imp_mean = SimpleImputer(missing_values=np.nan, strategy='mean').fit(train.X.astype(float)) # Fits the imoputer to replace NaN with column mean
    else: # GO and FUN datasets
    ...
    # Combine the features and the labels into tuples for each sample
    train_dataset = [(x, y) for (x, y) in zip(train.X, train.Y)] 
    
    # Appends the validation data to the training data IF NOT dataset_'others'
    if 'others' not in args.dataset: 
        val_dataset = [(x, y) for (x, y) in zip(val.X, val.Y)]
        # Comment the following loop to exclude the val_dataset from the training data 
        for (x, y) in zip(val.X, val.Y):
            train_dataset.append((x, y))
    test_dataset = [(x, y) for (x, y) in zip(test.X, test.Y)]

    # Apply incremental splitting to the training dataset if requested
    # From split ratios to actual splits
    if split_ratios:
        train_splits = []
        current_endpoint = 0
        for ratio in split_ratios:
            increment = int(len(train_dataset) * ratio)
            current_endpoint += increment # Update the endpoint
            current_endpoint = min(current_endpoint, len(train_dataset)) # Ensure endpoint within dataset lenght
            split = Subset(train_dataset, list(range(0, current_endpoint))) 
            train_splits.append(split)
        print(f"Generated {len(train_splits)} incremental splits: {[len(split) for split in train_splits]}")

        # If there are remaining samples, add a final split with all data
        # Not strictly necessary, but ensures all data is used when sum_splits is an odd number
        if current_endpoint < len(train_dataset):
            split = Subset(train_dataset, list(range(0, len(train_dataset))))
            train_splits.append(split)
            print(f"Added final split with {len(split)} samples to include all data.")
    else:
        train_splits = [train_dataset]  # No splitting, use full dataset
        print("No splitting applied; using the full dataset.")

    # Create DataLoader for the test set (remains unchanged)
    ...
    ####################### ---- MODIFIED FOR INCREMENTAL SPLITTING 


    # Initialize list for results
    results = []

    for split_idx, train_subset in enumerate(train_splits):
        print(f"\n=== Training on Split {split_idx + 1}/{len(train_splits)} with {len(train_subset)} samples ===")
        
        # Create DataLoader for the current split
        ...
        for epoch in range(num_epochs):
            ...
            # Iterate over batches in the current training loader
            for i, (x, labels) in enumerate(current_train_loader):
            ...
            avg_epoch_loss = epoch_loss / len(current_train_loader)
        ...
        with torch.no_grad():
            ...
            for i, (x, y) in enumerate(test_loader):
                ...
                # Move output and label back to CPU for sklearn processing
                predicted_test.append(predicted.cpu())
                constr_test.append(constrained_output.cpu())
                y_test.append(y.cpu())

            # Concatenate all batches
            predicted_test = torch.cat(predicted_test, dim=0)
            constr_test = torch.cat(constr_test, dim=0)
            y_test = torch.cat(y_test, dim=0)

            # Calculate average precision score
            score = average_precision_score(y_test[:, test.to_eval], constr_test.data[:, test.to_eval], average='micro')

            print(f"Average Precision Score for Split {split_idx + 1}: {score:.4f}")
    ...
    results_df.to_csv(f'results/{dataset_name}_split_results.csv', index=False)
    print("|n=== Experiment Completed ===")
    print(results_df)
